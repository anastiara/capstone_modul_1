...
# untuk menampilkan Submenu menu Menampilkan data pasien
    ...
    ******Menampilkan Data Pasien******
    ...
    3. Menampilkan data pasien tertentu berdasarkan Tanggal Lahir
...
def display_patient1():
    if data_pasien != []:
        ...
        for i in range(len(data_pasien)):
            print('{:<9} | {:<9} | {:<16} | {:<16} | {:<17}'.format(data_pasien[i]['ID'],data_pasien[i]['nama'],data_pasien[i]['tanggalLahir'],data_pasien[i]['jenisKelamin'],data_pasien[i]['status']))
        display_patient()
    elif data_pasien == []:
...
def display_patient3():
    if data_pasien == []:
        ...
        tanggalLahir_masukan = input('Masukkan Tanggal Lahir [contoh 19 Oktober 1995] :  ')
        tidak_ada = 0
        print('{:<9} | {:<9} | {:<16} | {:<16} | {:<17}'.format('ID','Nama','Tanggal Lahir','Jenis Kelamin','Status'))
        while True:
            for i in range(len(data_pasien)):
                if data_pasien[i]['tanggalLahir'] == tanggalLahir_masukan:        
                    print('{:<9} | {:<9} | {:<16} | {:<16} | {:<17}'.format(data_pasien[i]['ID'],data_pasien[i]['nama'],data_pasien[i]['tanggalLahir'],data_pasien[i]['jenisKelamin'],data_pasien[i]['status']))
                    tidak_ada -= tidak_ada
                else:
                    tidak_ada += 1
